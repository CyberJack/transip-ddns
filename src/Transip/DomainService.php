 ...
 * Class DomainService
 *
 * This class is a 1:1 copy of the Transip_DomainService class with the following changes:
 * - Replaced 'self::' with 'static::'
 * - Using project ApiSettings (which use the setting from config.php)
 * - replaced 'die()' with 'throw new Exception'
 *
 * @see Transip_DomainService
 * @package CyberJack\Transip\Transip
 */
class DomainService extends Transip_DomainService
{
	/**
	...
	public static function _getSoapClient($parameters = array())
	{
		...
		if(static::$_soapClient === null)
		{
			...
			$wsdlUri  = "https://{$endpoint}/wsdl/?service=" . static::SERVICE;
			try
			{
				static::$_soapClient = new SoapClient($wsdlUri, $options);
			}
			catch(SoapFault $sf)
			{
				throw new Exception("Unable to connect to endpoint '{$endpoint}'");
			}
			static::$_soapClient->__setCookie('login', $apiSettings::$login);
			static::$_soapClient->__setCookie('mode', $apiSettings::$mode);
		}

		$timestamp = time();
		$nonce     = uniqid('', true);

		static::$_soapClient->__setCookie('timestamp', $timestamp);
		static::$_soapClient->__setCookie('nonce', $nonce);
		static::$_soapClient->__setCookie('clientVersion', static::API_VERSION);
		static::$_soapClient->__setCookie('signature', static::_urlencode(static::_sign(array_merge($parameters, array(
			'__service'   => static::SERVICE,
			'__hostname'  => $endpoint,
	...
	protected static function _sign($parameters)
	{
		...
		$digest = static::_sha512Asn1(static::_encodeParameters($parameters));
	...
	protected static function _encodeParameters($parameters, $keyPrefix = null)
	{
		if(!is_array($parameters) && !is_object($parameters))
			return static::_urlencode($parameters);
		...
		foreach($parameters as $key => $value)
		{
			$encodedKey = is_null($keyPrefix)
				? static::_urlencode($key)
				: $keyPrefix . '[' . static::_urlencode($key) . ']';

			if(is_array($value) || is_object($value))
			{
				$encodedData[] = static::_encodeParameters($value, $encodedKey);
			}
			else
			{
				$encodedData[] = $encodedKey . '=' . static::_urlencode($value);
			}
		}
	...
	{
		return static::_getSoapClient(array_merge(array($domainNames), array('__method' => 'batchCheckAvailability')))->batchCheckAvailability($domainNames);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName), array('__method' => 'checkAvailability')))->checkAvailability($domainName);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName), array('__method' => 'getWhois')))->getWhois($domainName);
	}
	...
	{
		return static::_getSoapClient(array_merge(array(), array('__method' => 'getDomainNames')))->getDomainNames();
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName), array('__method' => 'getInfo')))->getInfo($domainName);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainNames), array('__method' => 'batchGetInfo')))->batchGetInfo($domainNames);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName), array('__method' => 'getAuthCode')))->getAuthCode($domainName);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName), array('__method' => 'getIsLocked')))->getIsLocked($domainName);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domain), array('__method' => 'register')))->register($domain);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName, $endTime), array('__method' => 'cancel')))->cancel($domainName, $endTime);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domain, $authCode), array('__method' => 'transferWithOwnerChange')))->transferWithOwnerChange($domain, $authCode);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domain, $authCode), array('__method' => 'transferWithoutOwnerChange')))->transferWithoutOwnerChange($domain, $authCode);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName, $nameservers), array('__method' => 'setNameservers')))->setNameservers($domainName, $nameservers);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName), array('__method' => 'setLock')))->setLock($domainName);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName), array('__method' => 'unsetLock')))->unsetLock($domainName);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName, $dnsEntries), array('__method' => 'setDnsEntries')))->setDnsEntries($domainName, $dnsEntries);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName, $registrantWhoisContact), array('__method' => 'setOwner')))->setOwner($domainName, $registrantWhoisContact);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName, $contacts), array('__method' => 'setContacts')))->setContacts($domainName, $contacts);
	}
	...
	{
		return static::_getSoapClient(array_merge(array(), array('__method' => 'getAllTldInfos')))->getAllTldInfos();
	}
	...
	{
		return static::_getSoapClient(array_merge(array($tldName), array('__method' => 'getTldInfo')))->getTldInfo($tldName);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domainName), array('__method' => 'getCurrentDomainAction')))->getCurrentDomainAction($domainName);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domain), array('__method' => 'retryCurrentDomainActionWithNewData')))->retryCurrentDomainActionWithNewData($domain);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domain, $newAuthCode), array('__method' => 'retryTransferWithDifferentAuthCode')))->retryTransferWithDifferentAuthCode($domain, $newAuthCode);
	}
	...
	{
		return static::_getSoapClient(array_merge(array($domain), array('__method' => 'cancelDomainAction')))->cancelDomainAction($domain);
	}
}
